...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:xs="http://www.w3.org/2001/XMLSchema" queryBinding="xslt2">
	...
	<title>Envanter defteri dokumanlarını kontrol etmek için gerekli olan schematron kuralları</title>
	<let name="periodCoveredStart" value="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:documentInfo/gl-cor:periodCoveredStart"/>
	<let name="periodCoveredEnd" value="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:documentInfo/gl-cor:periodCoveredEnd"/>
	<let name="vknTckn" value="/envanter:defter/xbrli:xbrl/xbrli:context/xbrli:entity/xbrli:identifier"/>
	...
	<pattern id="kok">
		<rule context="*">
			<assert test="not(@decimals) or  @decimals = 'INF' or @decimals = '2'">Belge içerisindeki 'decimals' nitelikleri 'INF' değerini almalıdır.</assert>
		</rule>
		<rule context="/">
			<assert test="envanter:defter">Envanter defteri dokumanı envanter:defter ana elemanı ile başlamalıdır.</assert>
		</rule>
	</pattern>
	<pattern id="entity">
		<rule context="/envanter:defter/xbrli:xbrl/xbrli:context/xbrli:entity">
			<assert test="not(xbrli:segment/gl-bus:numberOfEntries)"> xbrli:segment/gl-bus:numberOfEntries elemanı envanter defterinde bulunmamalıdır.</assert>
	...
	<pattern id="xbrl">
		...
		<rule context="/envanter:defter/xbrli:xbrl/xbrli:context/xbrli:entity/xbrli:identifier">
	...
	<pattern id="measure">
		<rule context="/envanter:defter/xbrli:xbrl/xbrli:unit/xbrli:measure">
			...
			<assert test="not(parent::node()[contains($currencyCodeList,@id)]) or  not(/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:documentInfo/gl-muc:defaultCurrency) or not(.= concat('iso4217:',parent::node()/@id)) or /envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:documentInfo/gl-muc:defaultCurrency=."> gl-muc:defaultCurrency değeri (<sch:value-of select="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:documentInfo/gl-muc:defaultCurrency"/>) hatalıdır. gl-muc:defaultCurrency elemanı varsa değeri xbrli:measure(<sch:value-of select="."/>) ile aynı olmalıdır.</assert>
		</rule>
	</pattern>
	<pattern id="accountingentries">
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries">
			...
			<let name="dosyaAdi" value="base-uri(.)"/>
			<assert test="not(contains($dosyaAdi,'E-000000')) or ($entryHeaderSayisi > 0)">Tüm ayı kapsayan bir defterde en az 1 gl-cor:entryHeader elemanı olmalıdır.</assert>
		</rule>
	</pattern>
	<pattern id="documentinfo">
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:documentInfo">
			<assert test="gl-cor:entriesType">gl-cor:entriesType zorunlu bir elemandır.</assert>
			<assert test="normalize-space(gl-cor:entriesType) = 'assets'">gl-cor:entriesType elemanı emvanter defteri için 'assets' değerini almalıdır.</assert>
			<assert test="gl-cor:uniqueID">gl-cor:uniqueID zorunlu bir elemandır.</assert>
			<assert test="not(gl-cor:uniqueID) or starts-with(normalize-space(gl-cor:uniqueID),'ENV')">gl-cor:uniqueID elemanı envanter defteri için 'ENV' değeri ile başlamalıdır.</assert>
	...
	<pattern id="entityinformation">
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:entityInformation">
		...
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:entityInformation/gl-bus:accountantInformation">
		...
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:entityInformation/gl-bus:entityPhoneNumber">
		...
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:entityInformation/gl-bus:entityEmailAddressStructure">
		...
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:entityInformation/gl-bus:entityWebSite">
	...
	<pattern id="entryheader">
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:entryHeader">
			...
			<assert test="not(following::node()) or not(following::node()/gl-cor:enteredDate) or not(gl-cor:enteredDate) or following::node()/gl-cor:enteredDate >= gl-cor:enteredDate">Envanter defteri tarihe göre sıralı olmalıdır.</assert>
	...
	<pattern id="entrydetail">
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:entryHeader/gl-cor:entryDetail">
...
<!--			<assert test="not(gl-muc:amountCurrency) or  /envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:documentInfo/gl-muc:defaultCurrency">gl-muc:amountCurrency olması durumunda gl-muc:defaultCurrency elemanı da bulunmalıdır </assert>-->
	...
	<pattern id="measurable">
		<rule context="/envanter:defter/xbrli:xbrl/gl-cor:accountingEntries/gl-cor:entryHeader/gl-cor:entryDetail/gl-bus:measurable">
			<let name="measureCodeList" value="',DAY,MON,ANN,HUR,D61,D62,C62,PA,BX,MGM,GRM,KGM,LTR,TNE,NT,GT,MMT,CMT,MTR,KTM,MLT,MMQ,CMK,CMQ,MTK,MTQ,KJO,CLT,KWH,MWH,CCT,D30,D40,LPA,B32,NCL,PR,R9,SET,T3,LM,AMVT,HAGR,CT,'"/>
			<let name="measureCode" value="substring(.,9)"/>
			<assert test="not (starts-with(normalize-space(.),'iso4217:')) or contains($measureCodeList, concat(',',$measureCode,','))">Gecersiz currency degeri: '<sch:value-of select="$measureCode"/>'.</assert>
			<assert test="not(gl-bus:measurableDescription) or gl-bus:measurableID">gl-bus:measurableID elemanı zorunludur.</assert>
<!--			<assert test="gl-bus:measurableDescription">gl-bus:measurableCodeDescription elemanı zorunludur.</assert>-->
			<assert test="not(gl-bus:measurableCodeDescription) or gl-bus_measurableQuantity">gl-bus:measurableQuantity elemanı zorunludur.</assert>
			<assert test="not(gl-bus:measurableCodeDescription) or gl-bus:measurableUnitOfMeasure">gl-bus:measurableQualifier elemanı zorunludur.</assert>			
		</rule>
	</pattern>
	<pattern id="binaryObject">
		<rule context="/envanter:defter/extensions/extension/defterek:binaryObject">
